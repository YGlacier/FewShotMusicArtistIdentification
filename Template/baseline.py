...
from .model import ArtistIdentificationFeatureModel, ArtistIdentificationClassifierModel

class Baseline(FewShotTemplate):
    def __init__(self, device, k_way, k_shot):
        super(Baseline, self).__init__(device, k_way, k_shot)
        self.feature_model = ArtistIdentificationFeatureModel().to(self.device)
        self.feature_model.eval()
        self.classifier_model = ArtistIdentificationClassifierModel(self.k_way).to(self.device)
